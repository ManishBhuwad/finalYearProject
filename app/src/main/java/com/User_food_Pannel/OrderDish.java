...
public class OrderDish extends AppCompatActivity {
    String RandomId, MessID;
    ImageView imageView;
//    ElegantNumberButton additem;
    TextView Foodname, MessName, MessLoaction, FoodQuantity, FoodPrice, FoodDescription;
    DatabaseReference databaseReference, dataaa, messdata, reference, data, dataref;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        MessName = (TextView) findViewById(R.id.mess_name);
        MessLoaction = (TextView) findViewById(R.id.mess_location);
...
//        additem = (ElegantNumberButton) findViewById(R.id.number_btn);
        ...
        dataaa.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                databaseReference = FirebaseDatabase.getInstance().getReference("FoodSupplyDetails").child(State).child(City).child(Sub).child(MessID).child(RandomId);
...
//                        Glide.with(OrderDish.this).load(updateDishModel.getImageURL()).into(imageView);
                        ...
                        messdata = FirebaseDatabase.getInstance().getReference("Mess").child(MessID);
                        messdata.addListenerForSingleValueEvent(new ValueEventListener() {
                            @Override
                            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                Mess mess = dataSnapshot.getValue(Mess.class);

                                String name = "<b>" + "Mess Name: " + "</b>" + mess.getFname() + " " + mess.getLname();
                                MessName.setText(Html.fromHtml(name));
                                String loc = "<b>" + "Location: " + "</b>" + mess.getSuburban();
                                MessLoaction.setText(Html.fromHtml(loc));
                                userrID = FirebaseAuth.getInstance().getCurrentUser().getUid();
                                databaseReference = FirebaseDatabase.getInstance().getReference("Cart").child("CartItems").child(userrID).child(RandomId);
                                databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
                                    @Override
                                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
